...
public class ToDoService {
	...
	public ResponseEntity<String> addTodoContent(int id, ContentsEntity contentsEntity) {
		...
		return new ResponseEntity<>("New Content Added!!", HttpStatus.CREATED);
	}
	...
	public ResponseEntity<String> updateTodoContent(int id, ContentsEntity contentsEntity) {
		...
		return new ResponseEntity<>("ToDo Content Updated!!", HttpStatus.OK);
	}

	public ResponseEntity<String> deleteTodoContent(int todoId, int contentId) throws Exception {

		boolean counter = false;
		ToDoEntity existingToDo =toDoRepository.findById(todoId).orElseThrow(() -> new ResourceNotFoundException("ToDo", "Id", Integer.toString(todoId)));
		...
		for(int i = 0 ; i < existingTodoContents.size() ; i++)
		{
			...
			{
				contentsRepository.deleteTodoContentMapping(contentId);
		...
		return new ResponseEntity<>("Content deleted for ToDo Id: " + todoId, HttpStatus.OK);
	}
	...
	public boolean deleteToDoMapping(ToDoEntity existingToDo, int todoId, int contentId) {
